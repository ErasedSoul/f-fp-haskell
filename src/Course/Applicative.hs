...
sequence ::
  ...
  List (k a)
  -> k (List a)
sequence a = foldRight (lift2 (:.)) (pure Nil) a  
...
replicateA n ka 
  | n == 0 = pure Nil
  | otherwise = (:. ) <$> ka <*> replicateA (n-1) ka
...
filtering ::
  ...
  -> List a
  -> k (List a)
--filtering p a = foldRight (\a acc -> lift2 (\b -> if b then (a:.acc) else acc) (p a)) (pure Nil) a
--filtering f a  = foldRight (\a acc ->  lif) (pure Nil) a 
--filtering f a  = foldRight (\a acc ->  lif) (pure Nil) a 
filtering f a = foldRight (\a acc -> (\x acc -> if x == True then (a:.acc) else acc) <$> (f a) <*> acc) (pure Nil) a
--filte 
